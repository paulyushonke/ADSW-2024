...
abstract class ReportFactory {
  abstract createReport(data: Employee[]): string;
}

class TxtReportFactory extends ReportFactory {
  createReport(data: Employee[]): string {
    return data
      .map(
        (emp) =>
          `${emp.name},${emp.gender},${emp.birthday},${emp.salary},${emp.progLang},${emp.importance}`,
      )
      .join("\n");
  }
}
  ...
  createReport(data: Employee[]): string {
    return `<employees>${data
      .map(
        ...
        <birthday>${emp.birthday}</birthday>
        <salary>${emp.salary}</salary>
        <progLang>${emp.progLang}</progLang>
        <importance>${emp.importance}</importance>
      </employee>`,
      )
      .join("")}</employees>`;
  }
...
export function EmployeeManagement({ employees }: { employees: Employee[] }) {
  ...
  const [fileType, setFileType] = useState<FileType>(FileType.TXT);
  ...
  const saveToFile = () => {
    ...
    switch (fileType) {
      ...
      case FileType.JSON:
        ...
        break;
      default:
        factory = new TxtReportFactory();
    }

    const report = factory.createReport(originator.getState().employees);
          ...
          <CardContent>
            <div className="flex items-center gap-4 pt-3">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value={FileType.TXT}>TXT</SelectItem>
            ...
            <div className="flex justify-between pt-4">
              <div className="mb-4 flex space-x-2">
                {/* <Button onClick={() => handleAction("START")}>To START</Button> */}
